...
type Shop = {
  name: string;
  slug?: string;
  shopCode?: string; // Added shopCode
  phone?: string;
  location?: string;
  hours?: string;
...
export default function DashboardScreen() {
  ...
  const handleCopyCode = async () => {
    ...
    await Clipboard.setStringAsync(shop.shopCode);
    setCodeCopied(true);
  ...
  const handleShareLink = async () => {
    ...
    try {
      await Share.share({
        message: `Join our queue at ${shop.name}!\n\nShop Code: ${shop.shopCode || 'N/A'}\nLink: ${link}`,
    ...
    <SafeAreaView className="flex-1 bg-gray-50 dark:bg-gray-900">
      ...
      <View className="flex-row items-start justify-between border-b border-gray-100 bg-white px-4 pb-4 pt-4 dark:border-gray-700 dark:bg-gray-800">
        <View className="mr-2 flex-1 flex-col">
          <Text className="text-xl font-bold text-black dark:text-white" numberOfLines={1} ellipsizeMode="tail">
            {shop.name}
          </Text>
        ...
        <View className="flex-col items-end">
          <View className={`mb-1.5 rounded-full px-3 py-1.5 ${shop.status === 'open' ? 'bg-emerald-600' : shop.status === 'frozen' ? 'bg-amber-600' : 'bg-red-600'}`}>
            <Text className="text-xs font-bold uppercase tracking-wide text-white">{shop.status || 'CLOSE'}</Text>
          </View>
          {shop.hours && (
      ...
      <ScrollView className="flex-1 px-2 pt-4" refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refreshData} />} showsVerticalScrollIndicator={false}>
        ...
        <View className="mb-5 rounded-2xl border border-gray-100 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          ...
          {/* Shop Code Section */}
          {shop.shopCode && (
             <View className="mb-4 flex-row items-center justify-between rounded-xl bg-orange-50 px-4 py-3 dark:bg-orange-900/20">
               <View>
                 <Text className="text-lg font-bold text-orange-600">Shop Code</Text>
                 <Text className="text-2xl font-black text-gray-800 dark:text-white">{shop.shopCode}</Text>
               </View>
               <TouchableOpacity 
                 onPress={handleCopyCode} 
                 className={`rounded-xl px-5 py-2.5 ${codeCopied ? 'bg-green-600' : 'bg-orange-500'}`}
               >
                 <Text className="text-lg font-bold text-white">{codeCopied ? 'Copied' : 'Copy'}</Text>
               </TouchableOpacity>
             </View>
          )}
          ...
          <View className="flex-row gap-3">
            <TouchableOpacity onPress={handleCopyLink} activeOpacity={0.7} className={`flex-1 flex-row items-center justify-center rounded-xl py-3 ${copied ? 'bg-green-600' : 'bg-blue-600'}`}>
              <Ionicons name={copied ? 'checkmark-circle' : 'copy-outline'} size={20} color="white" />
              <Text className="ml-2 text-lg font-bold text-white">{copied ? 'Copied!' : 'Copy Link'}</Text>
            </TouchableOpacity>

            <TouchableOpacity onPress={handleShareLink} activeOpacity={0.7} className="flex-1 flex-row items-center justify-center rounded-xl border border-gray-200 bg-gray-100 py-3 dark:border-gray-600 dark:bg-gray-700">
          ...
          <Text className="mt-3 text-left text-xl text-gray-500 dark:text-gray-400">
            Customers use this to join your queue.
          </Text>
        </View>

        {/* 3. Quick Actions Grid */}
        <View className="mb-5 flex-row gap-3">
          <TouchableOpacity className="flex-1 items-center rounded-2xl border border-gray-100 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800" onPress={() => navigation.navigate('Barbers' as never)}>
          ...
          <TouchableOpacity className="flex-1 items-center rounded-2xl border border-gray-100 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800" onPress={() => navigation.navigate('EditShop' as never)}>
        ...
        <View className="mb-5 rounded-2xl border border-gray-100 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          ...
          <View className="flex-row justify-between">
            <View className="flex-1 items-center">
              <Text className="text-2xl text-black dark:text-white">{loading && !refreshing ? '-' : totalToday}</Text>
              <Text className="mt-1 text-center text-xl text-gray-500 dark:text-gray-400">Total Customers Today</Text>
            </View>
            <View className="mx-1 h-16 w-[1px] self-center bg-gray-200 dark:bg-gray-700" />
            <View className="flex-1 items-center">
              <Text className="text-2xl text-emerald-500">{loading && !refreshing ? '-' : completed}</Text>
              <Text className="mt-1 text-center text-xl text-gray-500 dark:text-gray-400">Completed</Text>
            </View>
            <View className="mx-1 h-16 w-[1px] self-center bg-gray-200 dark:bg-gray-700" />
            <View className="flex-1 items-center">
              <Text className="text-2xl text-blue-500">{loading && !refreshing ? '-' : waiting}</Text>
              <Text className="mt-1 text-center text-xl text-gray-500 dark:text-gray-400">Waiting</Text>
            </View>
          </View>
        </View>
        ...
        <View className="mb-5 rounded-2xl border border-gray-100 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          ...
          <View className="mb-3 flex-row gap-3">
            <TouchableOpacity onPress={() => toggleShopStatus('open')} className={`flex-1 flex-row items-center justify-center rounded-xl border p-3 ${shop.status === 'open' ? 'border-emerald-200 bg-emerald-50 dark:border-emerald-700 dark:bg-emerald-900/40' : 'border-emerald-100 bg-white dark:border-emerald-800 dark:bg-gray-900/50'}`}>
            ...
            <TouchableOpacity onPress={() => toggleShopStatus('close')} className={`flex-1 flex-row items-center justify-center rounded-xl border p-3 ${shop.status === 'close' ? 'border-rose-200 bg-rose-50 dark:border-rose-700 dark:bg-rose-900/40' : 'border-rose-100 bg-white dark:border-rose-800 dark:bg-gray-900/50'}`}>
          ...
          <TouchableOpacity onPress={() => toggleShopStatus(shop.status === 'frozen' ? 'open' : 'frozen')} className={`w-full flex-row items-center justify-center rounded-xl border p-3 ${shop.status === 'frozen' ? 'border-amber-300 bg-amber-50 dark:border-amber-700 dark:bg-amber-900/40' : 'border-amber-200 bg-white dark:border-amber-800 dark:bg-gray-900/50'}`}>
            <MaterialCommunityIcons name="pause" size={20} color="#d97706" />
            <Text className="ml-2 text-xl font-bold text-amber-600">{shop.status === 'frozen' ? 'Unfreeze Queue' : 'Freeze Queue'}</Text>
          </TouchableOpacity>
        </View>
        ...
        <View className="mb-10 rounded-2xl border border-gray-100 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          ...
          {loading && !refreshing ? (
            ...
            <View className="items-center py-6">
              <Text className="text-xl text-gray-500 dark:text-gray-400">Queue is currently empty</Text>
            </View>
          ) : (
            queue
              ...
              .map((item, index) => (
                <View key={item._id} className={`flex-row items-start justify-between rounded-xl border border-gray-100 bg-gray-50 p-4 dark:border-gray-600 dark:bg-gray-700 ${index !== 0 ? 'mt-3' : ''}`}>
                  <View className="flex-1">
                    <Text className="mb-1 text-xl text-black dark:text-white">{item.name}</Text>
                    <View className="flex-col">
                      <Text className="mb-1 text-xl text-gray-500 dark:text-gray-400">Barber: {(typeof item.barber === 'object' ? item.barber?.name : item.barber) || 'Any Barber'}</Text>
                  ...
                  <View className="flex-row items-center rounded border border-gray-100 bg-white px-2 py-1 dark:border-gray-500 dark:bg-gray-600">
                    ...
                    <Text className="ml-1.5 text-xl font-medium text-blue-600">{item.serviceDuration} min</Text>
                  </View>
                </View>
              ))
          )}
